...
/**
 * Note that in this class we take great care to defer complicated calculations
 * to the end of the current transaction if the buffer informs us a compound
 * edit is in progress (<code>isTransactionInProgress()</code>).
 * 
 * This greatly speeds up replace all for example, by only doing certain things
 * once, particularly in <code>moveCaretPosition()</code>.
 * 
 * Try doing a replace all in a large file, for example. It is very slow in 3.2,
 * faster in 4.0 (where the transaction optimization was introduced) and faster
 * still in 4.1 (where it was further improved).
 * 
...
class BufferHandler implements BufferListener
{
	...
	{
		// System.err.println("foldLevelChanged " + (start-1) + " to " +
		// textArea.getLastPhysicalLine() + "," + end);
	...
	public void preContentRemoved(JEditBuffer buffer, int startLine, int offset, int numLines, int length)
	{
		...
		if (textArea.getDisplayManager() == displayManager)
		{
			...
			else
			{
				...
				if (offset != buffer.getLineStartOffset(startLine) || offset + length != buffer.getLineStartOffset(lastLine))
				{
					getReadyToBreakFold(startLine);
		...
		// scroll line count.
		else if (!displayManager.isLineVisible(firstLine.physicalLine))
		{
