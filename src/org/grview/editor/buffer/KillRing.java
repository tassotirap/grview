...
import org.grview.editor.gui.MutableListModel;

/**
 * The kill ring retains deleted text. This class is a singleton -- only one
 * kill ring is used for all of jEdit. Nothing prevents plugins from making
 * their own kill rings for whatever reason, though.
 */
public class KillRing implements MutableListModel
{
	...
	/**
	 * Since the kill ring has a wrap-around representation, we need to convert
	 * user-visible indices to actual indices in the array.
	 */
	...
	// {{{ reset() method
	/**
	 * This method is made to be used by implementation of load() method to
	 * initialize (or reset) the killring by a loaded sequence of objects.
	 * 
	 * Each element is converted to an element of the killring as followings: -
	 * If it is a String, it is converted as if it is a result of
	 * getElementAt(n).toString(). - Otherwise, it is converted as if it is a
	 * Object which was obtained by getElementAt(n).
	 * 
	...
	{
		/*
		 * This is not terribly efficient, but this method is only called by the
		 * 'Paste Deleted' dialog where the performance is not exactly vital
		 */
		remove(index);
