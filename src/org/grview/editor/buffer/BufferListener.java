 ...
 * <p>
 * 
 * This interface is new in jEdit 4.3pre3. The text area was made independent of
 * the rest of jEdit, and thus this class could no longer depend on
 * <code>org.gjt.sp.jedit.Buffer</code>.
 * <p>
 * 
 * While the {@link org.grview.actions.msg.BufferUpdate} EditBus message is used
 * for general buffer state changes, this interface is used for events which are
 * fired frequently, or for which performance is essential.
 * <p>
 * 
 * Because this interface is subject to change in the future, you should
 * subclass <code>BufferAdapter</code> instead of implementing it directly.
 * 
 * @author Slava Pestov
 * @version $Id$
 * @since jEdit 4.3pre3
...
public interface BufferListener
{
	...
	// {{{ foldHandlerChanged() method
	/**
	 * Called to notify the text area that folds need to be collapsed if the
	 * "collapseFolds" property is set. This method is called after the buffer
	 * has been loaded, and also if the user changes the fold handler.
	 * 
	 * @param buffer
	 *            The buffer in question
	 * @since jEdit 4.3pre3
	...
	/**
	 * Called when text is about to be removed from the buffer, but is still
	 * present.
	 * 
	 * @param buffer
	 *            The buffer in question
	...
	/**
	 * Called after an undo or compound edit has finished. The text area uses
	 * this event to queue up and collapse cleanup operations so they are only
	 * run once during a long transaction (such as a "Replace All" operation.)
	 * 
	 * @param buffer
	 *            The buffer in question
	 * @since jEdit 4.3pre3
