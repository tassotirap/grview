...
class InputMethodSupport extends TextAreaExtension implements InputMethodRequests, InputMethodListener
{
	...
	public void caretPositionChanged(InputMethodEvent event)
	{
		...
		{
			/*
			 * Cancel horizontal adjustment for composed text. FIXME: The
			 * horizontal offset may be beyond the max value of owner's
			 * horizontal scroll bar.
			 */
			owner.scrollToCaret(false);
		}
		/*
		 * Invalidate one more line below the caret because the underline for
		 * composed text goes beyond the caret line in some font settings.
		 */
	...
	public TextHitInfo getLocationOffset(int x, int y)
	{
		if (composedTextLayout != null)
		{
			...
			float local_y = y - origin.y - caret.y - composedTextLayout.getLeading() - composedTextLayout.getAscent();
	...
	public void inputMethodTextChanged(InputMethodEvent event)
	{
		...
		if (text != null)
		{
			...
			if (committed_count > 0)
			{
				...
				char c;
				int count;
				for (c = text.first(), count = committed_count; c != CharacterIterator.DONE && count > 0; c = text.next(), --count)
				{
					owner.userInput(c);
					committed.append(c);
				}
	...
	public void paintValidLine(Graphics2D gfx, int screenLine, int physicalLine, int start, int end, int y)
	{
		if (composedTextLayout != null)
		{
			...
			if (start <= caret && caret < end)
			{
				...
				int offset_to_baseline = height - fm.getLeading() - fm.getDescent();
