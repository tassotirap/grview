...
public class DisplayTokenHandler extends DefaultTokenHandler
{
	...
	private static boolean canMerge(Chunk c1, Chunk c2, Segment seg)
	{
		...
		return ((c1.style == c2.style) && ch1 != '\t' && ch2 != '\t' && (c1.length + c2.length <= MAX_CHUNK_LEN));
	} // }}}

	// {{{ createChunk() method
	private Chunk createChunk(byte id, int offset, int length, TokenMarker.LineContext context)
	{
		return new Chunk(id, offset, length, getParserRuleSet(context), styles, context.rules.getDefault());
	} // }}}
		// {{{ merge() method
	...
	public void handleToken(Segment seg, byte id, int offset, int length, TokenMarker.LineContext context)
	{
		...
		for (int splitOffset = 0; splitOffset < length; splitOffset += MAX_CHUNK_LEN)
		{
			...
			if (wrapMargin != 0.0f)
			{
				...
				{
					if (x > wrapMargin && end != null && seenNonWhitespace)
					{
						Chunk nextLine = new Chunk(endOfWhitespace, end.offset + end.length, getParserRuleSet(context));
	...
	// {{{ init() method
	/**
	 * Init some variables that will be used when marking tokens. This is called
	 * before {@link JEditBuffer#markTokens(int, TokenHandler)} to store some
	 * data that will be required and that we don't want to put in the
	 * parameters
	 * 
	 * @param styles
	 ...
	 * @param wrapMargin
	 */
	public void init(SyntaxStyle[] styles, FontRenderContext fontRenderContext, TabExpander expander, List<Chunk> out, float wrapMargin)
	{
		super.init();
