...
{
	/**
	 * The skew is the scroll count from the beginning of the line. Used with
	 * soft wrap.
	 */
	int skew;
	...
	void physDown(int amount, int screenAmount)
	{
		...
		{
			Log.log(Log.DEBUG, this, "physDown() start: " + physicalLine + ':' + scrollLine);
		}
		...
		if (Debug.SCROLL_DEBUG)
		{
			Log.log(Log.DEBUG, this, "physDown() end: " + physicalLine + ':' + scrollLine);
		}
	...
	void physUp(int amount, int screenAmount)
	{
		if (Debug.SCROLL_DEBUG)
		{
			Log.log(Log.DEBUG, this, "physUp() start: " + physicalLine + ':' + scrollLine);
		}
		...
		for (;;)
		{
			...
			else
			{
				...
				scrollLine -= displayManager.getScreenLineCount(prevPhysicalLine);
			}
		}

		if (Debug.SCROLL_DEBUG)
		{
			Log.log(Log.DEBUG, this, "physUp() end: " + physicalLine + ':' + scrollLine);
		}
	...
	void scrollUp(int amount)
	{
		...
		if (amount <= skew)
		{
			// the amount is less than the skew, so we stay in the same like,
			// just going
			// upper
			skew -= amount;
		}
	...
	public void changed()
	{
		...
		if (Debug.SCROLL_DEBUG)
		{
			Log.log(Log.DEBUG, this, "changed() before: " + physicalLine + ':' + scrollLine + ':' + skew);
		} // }}}
		...
		if (Debug.SCROLL_VERIFY)
		{
			...
			for (int i = 0; i < displayManager.getBuffer().getLineCount(); i++)
			{
				...
				verifyScrollLine += displayManager.getScreenLineCount(i);
			}
		...
		if (Debug.SCROLL_DEBUG)
		{
			Log.log(Log.DEBUG, this, "changed() after: " + physicalLine + ':' + scrollLine + ':' + skew);
		} // }}}
	} // }}}
