...
abstract class Anchor
{
	 ...
	 * 
	 * @param startLine
	 *            the start of the insert
	 * @param numLines
	 *            the number of insterted lines
	 */
	 ...
	 * 
	 * @param startLine
	 *            the first line of the removed content
	 * @param offset
	 *            the offset in the start line
	 * @param numLines
	...
	void preContentRemoved(int startLine, int offset, int numLines)
	{
		...
		// The removed content starts before the Anchor, we need to pull the
		// anchor up
		if (physicalLine >= startLine)
		{
			if (physicalLine == startLine)
			...
			else
			{
				...
				// Check the lines from the beginning of the removed content to
				// the end (or the physical
				// line of the Anchor if it is before the end of the removed
				// content

				// int loopStart = startLine + 1;

				// {{{ treatment if the beginning of the deleted content is
				// inside a physical line that has several line counts
				/*
				 * if (displayManager.isLineVisible(startLine)) { int
				 * screenLineCount =
				 * displayManager.screenLineMgr.getScreenLineCount(startLine);
				 * if (screenLineCount > 1) { int lineStartOffset =
				 * textArea.getLineStartOffset(startLine);
				 * 
				 * int startScreenLine =
				 * textArea.getScreenLineOfOffset(lineStartOffset); int
				 * deleteStartScreenLine =
				 * textArea.getScreenLineOfOffset(offset); if (startScreenLine
				 * != deleteStartScreenLine) { loopStart = startLine + 2;
				 * scrollLine -= screenLineCount - deleteStartScreenLine +
				 * startScreenLine; } } }
				 */
				// }}}
				...
				for (int i = startLine + 1; i <= end; i++)
				{
					...
					{
						scrollLine -= displayManager.screenLineMgr.getScreenLineCount(i);
					}
				}
	...
	{
		return getClass().getName() + '[' + physicalLine + ',' + scrollLine + ']';
	} // }}}
}
