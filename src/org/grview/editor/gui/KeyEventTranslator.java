...
public class KeyEventTranslator
{
	...
	public static class Key
	{
		 ...
		 * being a duplicate - generate any action on data. However, phantom key
		 * events may be necessary to notify the rest of the GUI that the key
		 * event, if it was not a phantom key event but a real key event, would
		...
		public boolean equals(Object o)
		{
			...
			{
				Key k = (Key) o;
				if (StandardUtilities.objectsEqual(modifiers, k.modifiers) && key == k.key && input == k.input)
				{
					return true;
				}
			}
