...
public class Analyzer extends Thread{
	...
	public Analyzer(TabGraphNode tbG[],TabNode tbT[],TabNode tbNt[],File fileIn,
						Yylex lex) {
		this.tabGraph = tbG;
		this.tabT = tbT;
		this.tabNT = tbNt;
	...
	public void run() {
		...
		sr =
			new SemanticRoutinesRepo(parseStack,tabT);
		...
		/*2     Inicia o percurso do grafo sintatico pelo primeiro nó apontado pelo simbolo não terminal inicial*/
		...
		while (continueSentinel) {
			...
			if (I != 0) {
				...
				if (tabGraph[I].isTerm())
					...
					if (tabGraph[I].getSim() == 0) {
						...
						if ((tabT[tabGraph[I].getSim()].getName())
							...
							/*2                          chamada da RS referenciada pelo nó terminal reconhecido*/
							sr.setCurrentToken(currToken);
							sr.execFunction(tabGraph[I].getSem());
							readNext();
							/*2                         Acabei de reconhecer um simbolo, posso esvaziar a pilhaNãoTerminal*/
							nTermStack.clear();
							...
							/*2                     Se o nó representado por I não é igual ao ultimo valor lido */
							if (tabGraph[I].getAlt() != 0)
								/*2                         Se o nó representado por I possui alternativa.*/
								/*2                             Faça o I representar essa alternativa*/
								I = tabGraph[I].getAlt();
							else {
								/*2                         Se o nó representado por I não possui alternativa*/
								if (nTermStack.empty()) {
									/*2                             Se a pilhaNãoTerminal está vazia, tenho que tratar o erro*/
									dealWithError(
										IU,
										toppsIU,
										currToken.m_charBegin,
										currToken.m_line);
								} else {
									/*2                             Se a pilhaNãoTerminal não está vazia.*/
									/*2                                 Desempilho então um nó da pilha, que no caso corresponde a uma não terminal,*/
									/*2                                 e atribuo à I o nó alternativo desse não terminal*/
									int alternative;
									alternative =
					...
					grViewStack.push(
						new GrViewStackNode(I, parseStack.size() + 1));
				...
				if (!grViewStack.empty()) {
					...
					/*2                 Desempilha o nó da pilha do analisador e coloco-o em grViewSNode*/
					grViewSNode = (GrViewStackNode) grViewStack.pop();
					/*2                 Desempilho todos os nós da parseStack que representam o não terminal reconhecido*/
					...
					I = grViewSNode.no;
					/*2                 Chamada a rotina semantica referenciada pelo nó não-terminal reconhecido*/
					sr.setCurrentToken(currToken);
					sr.execFunction(tabGraph[I].getSem());
					...
					if (currentSymbol.equals(new String("$"))) {
						/*2                 Se a ultima palavra lida for igual ao final de arquivo*/
						/*2                     Então reconheci a linguagem...*/
						//AppOutput.displayText("End of File");
						sucess = true;
					} else {
						/*2                 Se a ultima palavra lida não for igual ao final de arquivo*/
						/*2                      Então não reconheci a linguagem...*/
						AppOutput.displayText(
							"The fist non-teminal of the grammar has been recognized. ", TOPIC.Output);
						AppOutput.displayText(
							"But the end-of-file symbol has not been recognized.", TOPIC.Output);
						sucess = false;
					}
	...
	/*A rotina de tratamento de erro é chamada quando o sibolo lido não coincide com os simbolos esperados, nesse 
	 * caso, tentamos quatro técnicas de correção de erros são elas:
	 * a)Eliminação de um símbolo: Nesta estratégia supomos que o usuário cometeu o engano de escrever a cadeia
	 * de entrada com um simbolo a mais. Tal simbolo (o simbolo que acabou de ser lido) deve então ser eliminado.
	 * b)Inserção de um símbolo: Nessa estratégia supomos que o usuário cometeu o engano de omitir um símbolo que 
	 * deve ser inserido.
	 * c)Um símbolo trocado: Nesta estratégia supomos que o usuário escreveu um símbolo erradamente.
	 * d)Busca de Delimitador: Nesta estratégia, procuramos um simbolo que delimite o não terminal sendo procurado.*/
	private void dealWithError(int IX, int toppsIU, int column, int line) {
		int oldI = IX;
		String txt ="";
		Object lixo;
		/*o proximo bloco é responsável por imprimir na tela aonde o erro ocorreu*/
		try {
			if(fileIn != null) {
		...
		if (!estrategiaEliminacao(oldI, toppsIU, column, line)) {
			...
			if (!estrategiaInsercao(oldI, toppsIU, column, line)) {
				AppOutput.errorRecoveryStatus(
					"Inserting a symbol strategy has not succeeded\n");
				/*Tenta corrigir o erro utilizando a estratégia da troca*/
				if (!estrategiaTroca(oldI, toppsIU, column, line)) {
					AppOutput.errorRecoveryStatus(
						"Replacing a symbol stategy has not succeeded\n");
					/*Tenta corrigir o erro utilizando a estratégia da busca de Delimitador*/
					if (!estrategiaBuscaDelimitador(oldI,toppsIU,column,line)) {
						AppOutput.errorRecoveryStatus(
							"Searching delimiters strategy has not succeeded\n");
						/*Até esse ponto nenhuma das estratégias tiveram sucesso ao corrigir o erro, então o próximo
						 * simbolo da entrada é lido a rotina de tratamento de erros é chamada novamente. Dessa vez 
						 * para esse novo simbolo lido.*/
						this.readNext();
						if(currToken.m_text.equals("$"))
							continueSentinel = false;
						else this.dealWithError(oldI,toppsIU,currToken.m_charBegin,currToken.m_line);
					}
				}
			}
		}
	...
	private boolean estrategiaBuscaDelimitador(
		int IX,
		int topps,
		int column,
		int line) {
		...
		while (!grViewStack.empty() && !achou) {
			/*Ainda existem alternativas a seguir ou é necessario desempilhar um nó na pilha do analisador*/
	...
	private boolean estrategiaTroca(int IX, int toppsIU, int column, int line) {
		...
		while (IX != 0 && !achou) {
			...
			if (this.tabGraph[IX].isTerm()) {
				...
				while (IY != 0 && !achou) {
					...
					if (this.tabGraph[IY].isTerm()) {
						...
						if (this.tabGraph[IY].getSim() == 0) {
							...
							String temp =
								this.tabT[this.tabGraph[IY].getSim()].getName();
				...
				/*Nesse ponto a pilhaNaoTerminalY está vazia, portanto qualquer nó que tenha sido colocado
				 * na grViewStack na busca das alternativas do nó IY, já foi removido. Logo,  posso 
				 * desenpilhar um nó da pilha do analisador na busca de uma alternativa do nó IX sem 
				 * problemas pois se a nTermStack não está vazia, o topo da pilha do analisador representa
				 * um nó não terminal que foi utilizado para chegar até o nó IX.*/
				if (!achou)
		...
		if (achou) {
			...
			AppOutput.errorRecoveryStatus(
				"Action: This symbol has been replaced by "
					+ this.tabT[this.tabGraph[IX].getSim()].getName()+"\n");
		} else {
			currentSymbol = pastSymbol;
			/*volta um token na cabeça de leitura do analisador léxico.*/
			lex.pushback(lex.yylength());
			/*Faz com que a pilha do analisador volte a ser o que era antes de usar essa estratégia.*/
	...
	private boolean estrategiaInsercao(
		int IX,
		int toppsIU,
		int column,
		int line) {
			...
			if (this.tabGraph[IX].isTerm()) {
				...
				while (IY != 0 && !achou) {
					...
					/*sucessor é um não terminal*/
						...
						//topoaux2 = topoaux2 + 1;
						//this.asinStack.add(topoaux2,new GrViewStackNode(IY, topps + 2));
						this.grViewStack.push(new GrViewStackNode(IY, topps + 2));
						pilhaNaoTerminalY.push(new Integer(IY));
						IY = this.tabNT[this.tabGraph[IY].getSim()].getPrim();
					}
				...
				/*não terminal na sequencia dos esperados*/
				//topoaux1 = topoaux1 + 1;
				//this.asinStack.add(topoaux1,new GrViewStackNode(IX, topps + 1));
				this.grViewStack.push(new GrViewStackNode(IX, topps + 1));
				this.nTermStack.push(new Integer(IX));
				IX = this.tabNT[this.tabGraph[IX].getSim()].getPrim();
			}
		...
		if (achou) {
			...
			AppOutput.errorRecoveryStatus(
				"Action: "
					+ this.tabT[this.tabGraph[IX].getSim()].getName()
					+ " inserted before column "+column+"\n");
		}
		/*deixa a pilha do Analisador como estava antes de começar essa estratégia*/
		else {
			while (this.grViewStack.size() > toppsIU) {
				Object lixo;
				lixo = this.grViewStack.pop();
	...
	private boolean estrategiaEliminacao(
		int IX,
		int toppsIU,
		int column,
		int line) {
		...
		readNext();
		/*Tenta achar um terminal que seja igual ao simbolo lido, percorrendo as alternativas do nó atual*/
		while (IX != 0) {
			/*eh terminal?*/
			if (tabGraph[IX].isTerm()) {
				...
				if (tabT[tabGraph[IX].getSim()].getName().equals(currentSymbol)) {
					...
					IU = I;
					toppsIU = this.parseStack.size();
					/*imprimir mensagem que o penultimo simbolo lido foi ignorado*/
					...
					int alternative = 0;
					alternative =
						this.findAlternative(IX, nTermStack, grViewStack);
					IX = alternative;
				}
			} else {
	...
	/*Esta função lê o proximo token de entrada, guardando o token em currToken e o simbolo correspondente em 
	  simbolo Atual */
	...
	/*Esta função é chamada quando o nó atual não possui como alternativa um terminal mas sim um não terminal.
	 * Então percorremos os não terminais guardados na pilha nãoTerminal até achar um simbolo terminal. Como
	 * tal simbolo é alternativa do nó sendo visitado, retornamos ele. Se não existe tal simbolo, retornamos "0"
	 * para indicar que o nó não possui alternativas. 
	 */
	private int findAlternative(int IZ, Stack pilhaNaoTerm, Stack pilhaAna) {
		...
		while (alternative == 0 && !pilhaNaoTerm.empty()) {
			trash = pilhaAna.pop();
			alternative =
				tabGraph[((Integer) pilhaNaoTerm.pop()).intValue()].getAlt();
		}
		return alternative;
	}
