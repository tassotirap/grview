...
public class TextUtilities
{
	 ...
	 * offset of the buffer, or -1 if the bracket is unmatched (or if the
		...
		for (int i = pos; i < line.length(); i++)
		{
			...
			// {{{ Whitespace...
				...
				case WORD_CHAR:
					...
					// (don't stop at every upper case char, don't treat
					// noWordSep as word chars)
					if (camelCasedWords && i > pos + 1 && !Character.isUpperCase(ch) && Character.isLetterOrDigit(ch) && Character.isUpperCase(lastCh))
					{
						return i - 1;
					}
					// stop at next first upper case char if camel caseg (don't
					// stop at every upper case char)
	...
	{
		return findWordEnd((CharSequence) line, pos, noWordSep, joinNonWordChars, camelCasedWords, eatWhitespace);
	} // }}}
		...
		for (int i = pos; i >= 0; i--)
		{
			...
			// {{{ Whitespace...
				case WHITESPACE:
					...
					// {{{ Word character...
				case WORD_CHAR:
					// stop at next last (in writing direction) upper case char
					// if camel cased
					// (don't stop at every upper case char, don't treat
					// noWordSep as word chars)
					...
					// stop at next first (in writing direction) upper case char
					// if camel cased
	...
	{
		return findWordStart((CharSequence) line, pos, noWordSep, joinNonWordChars, camelCasedWords, eatWhitespace);
	} // }}}

	// {{{ format() method
	/**
	 * Formats the specified text by merging and breaking lines to the specified
	 * width.
	 * 
	 * @param text
	 *            The text
	...
	public static String format(String text, int maxLineLength, int tabSize)
	{
		...
		for (;;)
		{
			...
			formatParagraph(text.substring(index, newIndex), maxLineLength, tabSize, buf);
			buf.append("\n\n");
			index = newIndex + 2;
		}
