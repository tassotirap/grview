 ...
 * <p>
 * 
 * A {@link Registers.Register} is string of text indexed by a single character.
 * Typically the text is taken from selected buffer text and the index character
 * is a keyboard character selected by the user.
 * <p>
 * 
 * This class defines a number of static methods that give each register the
 * properties of a virtual clipboard.
 * <p>
 * 
 * Two classes implement the {@link Registers.Register} interface. A
 * {@link Registers.ClipboardRegister} is tied to the contents of the system
 * clipboard. jEdit assigns a {@link Registers.ClipboardRegister} to the
 * register indexed under the character <code>$</code>. A
 * {@link Registers.StringRegister} is created for registers assigned by the
 * user. In addition, jEdit assigns <code>%</code> to the last text segment
 * selected in the text area. On Windows this is a
...
public class Registers
{
		...
		public String toString()
		{
			try
			{
				...
				if (false)
				{
					 ...
					 * 
					 * Apparently, jEdit is unable to copy text from clipbard
					 * into the current text buffer if the clipboard was filles
					 * using the command echo test | xselection CLIPBOARD -
					 * under Linux. However, it seems that Java does not offer
					 * any data flavor for this clipboard content (under J2RE
					 * 1.5.0_06-b05) Thus, copying from clipboard seems to be
					 * plainly impossible.
					 */
	...
	public static class StringRegister implements Register
	{
		...
		/**
		 * Called when this register is no longer available. This implementation
		 * does nothing.
		 */
		public void dispose()
		{
	 ...
	 *            The register
	 * @param separator
	 *            The text to insert between the old and new text
	 * @param cut
	 *            Should the current selection be removed?
	 * @since jEdit 3.2pre1
	 */
	...
	/**
	 * Returns a string of all defined registers, used by the status bar (eg,
	 * "a b $ % ^").
	 * 
	 * @since jEdit 4.2pre2
	 */
	...
	/**
	 * Returns an array of all available registers. Some of the elements of this
	 * array might be <code>null</code>.
	 */
