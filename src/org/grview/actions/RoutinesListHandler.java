...
public class RoutinesListHandler extends ActionListHandler
{
	...
	public void endElement(String uri, String localName, String qName)
	{
		...
		if (qName.equals(tag))
		{
			...
			{
				String selected = (getIsSelected().length() > 0) ? getIsSelected().toString() : null;
				AbstractEditAction action = getActionSet().createBeanShellAction(getActionName(), getCode().toString(), selected, isNoRepeat(), isNoRecord(), isNoRememberLast());
